...
static void DrawStringA(float fontSize, int x, int y, RGBA* color, bool bCenter, bool stroke, const char* pText, ...)
{
	va_list va_alist;
	char buf[1024] = { 0 };
...
static void DrawStringNoBackground(int x, int y, RGBA* main_color, const char* pText, ...)
{
	va_list va_alist;
	char buf[1024] = { 0 };
	va_start(va_alist, pText);
...
static void text(const ImVec2& v2_pos, const std::string& str_text, const ImVec4& v4_col)
{
	auto* draw_list = ImGui::GetOverlayDrawList();

	const char* text = str_text.c_str();

	if (!draw_list || !text || !*text)
		return;

	draw_list->AddText(ImVec2(v2_pos.x - 1, v2_pos.y - 1), ImGui::GetColorU32(ImVec4(0, 0, 0, v4_col.w)), text);
	draw_list->AddText(ImVec2(v2_pos.x + 1, v2_pos.y - 1), ImGui::GetColorU32(ImVec4(0, 0, 0, v4_col.w)), text);
	draw_list->AddText(ImVec2(v2_pos.x - 1, v2_pos.y + 1), ImGui::GetColorU32(ImVec4(0, 0, 0, v4_col.w)), text);
	draw_list->AddText(ImVec2(v2_pos.x + 1, v2_pos.y + 1), ImGui::GetColorU32(ImVec4(0, 0, 0, v4_col.w)), text);

	draw_list->AddText(v2_pos, ImGui::GetColorU32(v4_col), text);
}

static void DrawPlayerText(int x, int y, RGBA* main_color, const char* pText, ...)
{
	va_list va_alist;
	char buf[1024] = { 0 };
	va_start(va_alist, pText);
	...
	DrawNewText(x, y, main_color, text.c_str());
}